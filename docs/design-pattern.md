...
- [`../dags/`](../dags/) + [`../config/`](../config/) + [`../sql/`](../sql/) — the same real implementation, orchestrated by Airflow instead (`dags/order_lookup_sample.py` calls the same `pipeline/` functions)
