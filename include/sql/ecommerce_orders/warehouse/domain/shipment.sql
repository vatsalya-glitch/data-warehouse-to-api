...
TRUNCATE TABLE staging.sample_shipment;

INSERT INTO staging.sample_shipment
SELECT
    order_id,
    carrier,
    tracking_number,
...
FROM raw.ecommerce_shipment_events
WHERE order_id IS NOT NULL
QUALIFY ROW_NUMBER() OVER (PARTITION BY order_id ORDER BY event_timestamp DESC) = 1;

-- TODO: replace with real shipment source, columns, and dedup tie-breaker
